		...
		if(player != null) {
			if(!held.isEmpty() && held.getItem() != null) {
				...
				if(held.getItem() == Items.STICK) {
					...
					// BlockPattern eventPattern =
					// FactoryBlockPattern.start().aisle(new String[]
					// { "BBBBB", "BBBBB", "BBBBB", "BBBBB", "BBBBB",
					// }).where('B',
					// BlockWorldState.hasState(BlockStateMatcher.forBlock(world.getBlockState(pos).getBlock()))).build();
					// BlockPattern pattern = eventPattern;
					// BlockPattern.PatternHelper patternhelper =
					// pattern.match(world, pos);
					//
					...
					// for(int l = 0; l < eventPattern.getThumbLength(); ++l)
					// {
					// world.destroyBlock(patternhelper.translateOffset(k, l,
					// 0).getPos(), false);
					// }
					// }
					// event.setUseItem(Result.ALLOW);
				...
				// TODO ハシゴを下から伸ばせるように
				// if(held.getItem() == Item.getItemFromBlock(Blocks.LADDER) &&
				...
				// Blocks.LADDER.canPlaceBlockAt(world, posu)*/)
				// {
				// player.swingArm(hand);
				// world.playSound(player, pos, SoundEvents.BLOCK_LADDER_PLACE,
				// SoundCategory.BLOCKS, 1.0F, 0.8F);
				// if(!world.isRemote)
				// {
				// world.setBlockState(posu,
				// Blocks.LADDER.getDefaultState().withProperty(BlockLadder.FACING,
				// state.getValue(BlockLadder.FACING)));
				// if(!Utils.isCreative(player))
				// {
				// held.stackSize--;
				// }
	...
	public void onItemRightClick(PlayerInteractEvent.RightClickItem event) {
		...
		if(player != null) {
			if(!held.isEmpty() && held.getItem() != null) {
				// if(held.getItem() == Items.STICK)
				// {
				// player.addVelocity(player.motionX * 50, player.motionY * 50,
				// player.motionZ * 50);
				// }
				...
				// ItemStack stack = new ItemStack(item, 1, mxdmg > 0 ?
				// random.nextInt(Math.abs(mxdmg)) : 0);
