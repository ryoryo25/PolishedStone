...
//				if(dropHead)
			...
			/*
			//TODO レアドロップ
			if(((event.source.getEntity() instanceof EntityPlayer)) && (event.recentlyHit))
			{
			    EntityOcelot mob = (EntityOcelot) event.entity;

			    if(event.entityLiving instanceof EntityOcelot)
			    {
			        if ((!mob.isTamed()) || (mob.getTameSkin() != 1) || (mob.bi()))
			        {
			            return;
			        }
			        if (random.nextInt(100) < event.lootingLevel * 10 + 5)
			        {
			            event.entityLiving.entityDropItem(new ItemStack( ItemsCore.mobDrop, 1, 3), 1);
			        }
			    }
			    ...
			    {
			        BiomeGenHell biome = mob.k.a((int)mob.o, (int)mob.q);
			        if ((biome != abn.j) || (!mob.T()) || (mob.bi()))
			        {
			            return;
			        }
			        if (!mob.v_().equals("texture/crow.png"))
			        {
			            return;
			        }
			        if (random.nextInt(100) < event.lootingLevel * 3 + 1)
			        {
			            event.entityLiving.entityDropItem(new ItemStack( ItemsCore.mobDrop, 1, 4), 1);
			        }
			    }
			}
			    }*/
			/*
			    public boolean onEntityLivingUpdate(acq entity)
			    {
			if ((entity instanceof rd))
			{
			    ady nbt = entity.getEntityData();
			    if (nbt.c("texture"))
			    {
			        String tex = nbt.j("texture");
			        if (!entity.v_().equals(tex))
			        {
			            CE.setEntityTexture(entity, tex);
			        }
			    }
			}
			return false;
