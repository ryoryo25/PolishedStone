...
public class TileEntitySlimeRemovalTorch extends TileEntity implements ITickable
{
	...
	public void update()
	{
		...
		if(tickCounter == 10)
		{
			...
			// same meaning
//			this.world.getEntitiesWithinAABB(EntitySlime.class, search).stream().forEach(target -> target.setDead());

//			List<EntitySlime> slimes = this.world.getEntitiesWithinAABB(EntitySlime.class, search);
