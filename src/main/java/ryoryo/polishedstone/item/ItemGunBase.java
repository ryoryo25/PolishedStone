...
//		{
//			ammos[li] = (Item) Item.REGISTRY.getObject(new ResourceLocation(bullets[li]));
//		}
//	}
//
//	public void playSoundEmpty(World world, EntityPlayer player, ItemStack gun)
//	{
//		world.playSound(player, player.posX, player.posY, player.posZ, soundEmpty, SoundCategory.PLAYERS, volume, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
//	}
//
//	public void playSoundRelease(World world, EntityPlayer player, ItemStack gun)
//	{
//		world.playSound(player, player.posX, player.posY, player.posZ, soundRelease, SoundCategory.PLAYERS, volume, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
//	}
//
//	public void playSoundReload(World world, EntityPlayer player, ItemStack gun)
//	{
//		world.playSound(player, player.posX, player.posY, player.posZ, soundReload, SoundCategory.PLAYERS, volume, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
//	}
//
...
//	 * @param pState
//	 */
//	public void onFireTick(ItemStack gun, World world, EntityPlayer player, int count, int state)
//	{
//		if(isBurst(gun))
//		{
...
//	 * @return 射撃後の残弾数
//	 */
//	public int fireBullet(ItemStack gun, World world, EntityPlayer player, int useCount)
//	{
...
//			if(libullet != null)
//			{
//				lentity = libullet.getBulletEntity(gun, lbullet, world, player, 72000 - useCount);
...
//	 * @param pUseCount
//	 */
//	public void onRecoile(ItemStack gun, ItemStack bullet, World world, EntityPlayer player, int useCount)
//	{
//		// しゃがみの時は少し早く照準が安定する
//		float lsn = player.isSneaking() ? 0.5F : 1.0F;
//		lsn *= ((ItemBulletBase) bullet.getItem()).getReaction(bullet);
//		// 腕の動き
//		player.rotationPitch -= (player.getRNG().nextFloat() * getStabilityP(gun, player, useCount)
//				+ getStabilityPO(gun, player, useCount)) * lsn;
//		player.rotationYaw += (player.getRNG().nextFloat() * getStabilityY(gun, player, useCount)
//				+ getStabilityYO(gun, player, useCount)) * lsn;
//		// 後ろに吹っ飛ぶ
//		lsn *= getStability(gun, player, useCount);
//		player.motionX += MathHelper.sin(player.rotationYawHead * 0.01745329252F) * lsn;
//		player.motionZ -= MathHelper.cos(player.rotationYawHead * 0.01745329252F) * lsn;
//	}
//
//	/**
//	 * 装填されている弾を返す。
